create table `Block` (
  ...
  `preHash` varchar(64) NOT NULL comment 'Prev block hash',
...
add
  index `transaction_block_index`(`block`);
alter table `Transaction`
add
  index `transaction_txhash_index`(`txHash`);
...
alter table `Receipt`
add
  primary key `Receipt_pkey`(`txHash`);
create table `Event` (
    `data` text not null,
...
add
  unique `blockvalidator_address_version_unique`(`address`, `version`);
create table `Account` (`address` varchar(40) NOT NULL);
alter table `Account`
add
  primary key `Account_pkey`(`address`);
create table `Asset` (
...
add
  index `transfer_asset_index`(`asset`);
alter table `Transfer`
add
  index `transfer_from_index`(`from`);
alter table `Transfer`
add
  index `transfer_to_index`(`to`);
alter table `Transfer`
add
  index `transfer_txhash_index`(`txHash`);
alter table `Transfer`
add
  unique `transfer_txhash_unique`(`txHash`);
alter table `Transfer`
add
